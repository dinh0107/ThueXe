...
namespace ThueXe.Controllers
{
    public class SitemapController : Controller
    {
        ...
        public ContentResult ProductImageSitemap()
        {
            ...
            var articles = _unitOfWork.ProductRepository.GetQuery(a => a.Active && a.Image != null, q => q.OrderBy(a => a.Id)).Select(a => new
            {
                url = a.Url,
                image = a.Image
            });
            ...
            foreach (var article in articles)
            {
                ...
                if (article.image != null)
                {
                    images.Add(Request.Url?.GetLeftPart(UriPartial.Authority) + Path.Combine("/images/products/", article.image));
                }
